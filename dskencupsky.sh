...
for i in `cat /tmp/$DSKENCNAME.upload`;do curl -X POST "https://siasky.net/skynet/skyfile" -F "file=@$DSKPATH/split/$i" >> $DSKPATH/files_uploaded.siasky | echo "Uploading $DSKPATH/$DSKENCNAME/$i" >> $DSKPATH/files_uploaded.siasky;done
...
for i in `cat $DSKPATH/files_uploaded.siasky |awk -F 'skylink":"' '{ print $2}'|awk -F '","merkleroot' '{ print $1}'`;do curl "https://siasky.net/$i" -o $DSKPATH/download/$i;done
